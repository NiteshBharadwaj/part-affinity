    ...
    with tqdm(total=nIters) as t:
        for i, (input_, heatmap, paf, ignore_mask, indices) in enumerate(data_loader):
            ...
            for i in range(len(heatmap_outputs)):
                ...
                loss_paf_total += criterion_hm(paf_out * allow_mask, paf_t_cuda * allow_mask)
            ...
            if to_train:
                ...
                outputs.append((heatmap_outputs[-1].data.cpu().numpy(), paf_outputs[-1].data.cpu().numpy(), indices.numpy()))
...
def train_net(train_loader, test_loader, model, criterion_hm, criterion_paf, optimizer,
              n_epochs, val_interval, learn_rate, drop_lr, save_dir):
    heatmap_loss_avg, paf_loss_avg = 0.0, 0.0
    for epoch in range(1, n_epochs + 1):
        step(train_loader, model, criterion_hm, criterion_paf, True, optimizer)
        if epoch % val_interval == 0:
            test_net(test_loader, model, criterion_hm, criterion_paf, save_dir, epoch)
        adjust_learning_rate(optimizer, epoch, drop_lr, learn_rate)
    return heatmap_loss_avg, paf_loss_avg


def test_net(test_loader, model, criterion_hm, criterion_paf, save_dir=None, epoch=0):
    heatmap_loss_avg, paf_loss_avg, outputs = step(test_loader, model, criterion_hm, criterion_paf)
